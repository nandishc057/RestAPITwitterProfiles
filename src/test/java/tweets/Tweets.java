...
public class Tweets {
	...
	public void getLatestTweet() {
		...
		Response res = given().auth().oauth(consumerKey, consumerSecret, accessToken, secretToken)
				.queryParam("count", 1).when().get("/home_timeline.json").then().assertThat().statusCode(200).extract()
				.response();
	...
	public void postTweet() throws InterruptedException {
		RestAssured.baseURI = "https://api.twitter.com/1.1/statuses";
		Response res = given().auth().oauth(consumerKey, consumerSecret, accessToken, secretToken)
				.queryParam("status", "Hello Twitter2").when().post("update.json").then().assertThat().statusCode(200)
				.extract().response();
